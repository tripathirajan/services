...
describe("Encryption: base64Decode and base64Encode", () => {
  test("Before encode and after decode should be same", async () => {
    expect(await Encryption.base64Encode("testingEncodeBase64")).toEqual(
      "dGVzdGluZ0VuY29kZUJhc2U2NA=="
    );
    expect(
      await Encryption.base64Decode("dGVzdGluZ0VuY29kZUJhc2U2NA==")
    ).toEqual("testingEncodeBase64");
  });
});
